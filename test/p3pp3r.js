...
// state.initialise(model, instance,undefined);
state.Queue.push({name:'aa',run:function(cb){
    state.initialise(model, instance,undefined,cb);   
}})
state.Queue.push({name:'bb',run:function(cb){
    state.evaluate(model, instance, "event2",true,cb);
    // console.log("adfasdf");
}})
